...
function roundToPowerOfTen(num, pow) {
  const base = num / 10 ** pow; // делим число на заданную степень десяти
  const round = Math.round(base); // округляем полученное число
  return round * 10 ** pow; // возвроащаем разрядность числа
}
